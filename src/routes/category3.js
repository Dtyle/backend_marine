...
module.exports = async function (app) {
  const repo = app.db.getRepository("Category3");
  ...
  app.get("/category3", async (req, res) => {
    try {
      ...
      if (category2_id) {
        qb.andWhere("category.category2_id = :category2Id", {
      ...
      qb.leftJoin("category.category2", "category2");
      ...
      dataToSend.categories3 = results;
  ...
  app.post("/category3", async (req, res) => {
    if (
    ...
    try {
      const { code, name, sortOrder, category1_id, category2_id } = req.body;
      ...
      app.logAudit(req, payload.id, "category3", "create", {
        code,
        name,
        sortOrder,
        category1_id,
  ...
  app.put("/category3/:id", async (req, res) => {
    if (!validateRequiredFields(req, res, [{ f: "name", t: "string" }])) return;

    try {
      const { id } = req.params;
      const { name, sortOrder, category1_id, category2_id } = req.body;
      ...
      app.logAudit(req, id, "category3", "update", {
        name,
        sortOrder,
        category1_id,
        category2_id,
  ...
  app.delete("/category3/:id", async (req, res) => {
    try {
      const { id } = req.params;
      await repo.update({ id }, { status: "D" });

      app.logAudit(req, id, "category3", "inactivate", {
        status: "D",
      });
      res.send({ success: true });
    } catch (err) {
